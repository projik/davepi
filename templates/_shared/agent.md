...
| Passwordless email magic-link login, email invites | `davepi-plugin-magic-link` | `/auth/magic-link/{request,verify,invite}` routes; single-use SHA-256-hashed TTL tokens, enumeration-safe 204s, links sent via the framework mailer, issues the standard JWT on verify. Generic invite metadata guarded by a host-registered authoriser. |
