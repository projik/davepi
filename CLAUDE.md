...
**Role-based ACL is opt-in per schema.** `utils/acl.js` enforces field-level read/create/update visibility plus document-level `list`, `delete`, and `write` bypasses. Roles travel in the JWT (`req.user.roles` / `ctx.user.roles`); the User model defaults to `['user']`. A schema can declare `field.acl.{read,create,update}` and `schema.acl.{list,delete,write,scope}`; schemas without an `acl` key behave exactly as before (owner-only). REST and GraphQL wrappers both call into the same helpers, so coverage is symmetric. `wrapFilter`, `wrapByIdMutation`, etc. take an options object — `{ schema, kind: 'read'|'delete'|'write', action: 'create'|'update' }` — to choose which ACL slot applies. `kind` defaults to `'write'`, whose bypass slot is `schema.acl.write`: roles listed there may update records owned by another user (REST `PUT /{path}/:id`, the GraphQL `UpdateById`/`UpdateOne`/`UpdateMany` resolvers, the state-machine transition mutations, and MCP `update_{path}`), while tenant fields are still stripped from the payload so the record's owner never changes and field-level `update` ACL still applies on top. `wrapStateTransition` honours the same slot via `kind === 'write'`, so the transition surface stays consistent with `UpdateById`. The bulk REST `PUT /{path}` upsert stays owner-only regardless, since its filter doubles as the insert seed and a bypass there would mint ownerless docs. There is no document-level `create` bypass — create always stamps ownership from the JWT.

**Public read via client IDs.** Frontends without a JWT (storefronts, marketing sites, embedded widgets) read collections by sending an `X-Client-Id` header. `middleware/clientAuth.js` resolves the header against the `apiClient` collection into a synthetic `req.user = { user_id: <id>, roles: [<client.role>], isClient: true }`; from there, the existing ACL surface governs visibility. A new `schema.acl.scope[role] = { ...mongoFilter }` slot declares a **server-controlled mandatory filter** that's `$and`-ed into every read for that role and cannot be widened by the caller — that's how you expose "only published products" or "only future appointments" to a public role. Writes from client-authed callers are refused (REST 403 in `clientAuth`, GraphQL refuses `user.isClient` in `wrapCreateOne` / `wrapCreateMany` / `wrapByIdMutation` / `wrapStateTransition` / mutating `wrapFilter`). Bearer wins when both headers are present. The SDK exposes this as a `clientId` option on `createDavepiClient`. Client IDs are public identifiers, not secrets — bake them into SPA bundles, rotate by flipping the row's `status` to `revoked`.
...
- Apollo Server v5 (`@apollo/server` + `@as-integrations/express4`), not v3. Context lives on `expressMiddleware`, not the server constructor; `await server.start()` runs before the handler is mounted (the loader's single-flight queue serialises this). `introspection` is gated on `NODE_ENV !== 'production'` — with introspection on, v4+ serves the embedded Apollo Sandbox (the successor to v3's GraphQL Playground) from its default landing-page plugin; there's no `playground` option anymore. Typed GraphQL errors are minted via `utils/graphqlErrors.js` (a thin `GraphQLError` shim reproducing the v3 codes `UNAUTHENTICATED` / `FORBIDDEN` / arbitrary-coded `ApolloError`), since v4+ dropped the `apollo-server-errors` classes.
